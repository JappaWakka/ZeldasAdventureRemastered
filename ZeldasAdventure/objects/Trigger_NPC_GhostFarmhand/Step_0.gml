if IsPlayerOnSameTile() = true
{
	if global.FadeAlpha = 0 and Register_Registered("Farmhand_Dialogue") = false
	{
		...
		{
			global.CurrentDialogue_Asset = Dialog_GreatWimbich_GhostFarmer_WaterShrine_Classic
	...
	if global.CurrentDialogue_Asset != Dialog_None
	{
		if IsMenuVisible() = false
		{
			...
			if global.CurrentDialogue_Asset = Dialog_GreatWimbich_GhostFarmer_WaterShrine_Classic
			{
				global.Subtitle = Subtitle_GreatWimbich_GhostFarmer_WaterShrine_Classic(AudioPosition)
			}
		}
		else
		{
