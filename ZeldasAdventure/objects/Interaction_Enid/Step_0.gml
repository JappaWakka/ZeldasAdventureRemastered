...
{
	if global.CurrentDialogue_Asset = Dialog_None and place_meeting(x,y,Entity_Player) and Register_Registered(Registers.Enid_StolenHeart) = false
	{
		if global.RemasteredMode = false or Register_Registered(Registers.Enid_HitWithSpell) = false
		{
			...
			Register_Add(Registers.Enid_StolenHeart)
		}
		else
		{
	...
	{
		if global.RemasteredMode = true and Register_Registered(Registers.Enid_HitWithSpell) = false
		{
			global.CurrentDialogue_Asset = Dialog_ForestOfOgham_14_Enid_TooClever
			global.CurrentDialogue_ID = audio_play_sound_relative_toentity(Entity_NPC_Enid, global.CurrentDialogue_Asset,500,false)
			Register_Add(Registers.Enid_HitWithSpell)
		}
	}
}